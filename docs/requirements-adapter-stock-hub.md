...
| `decliners` | `--top N`（既定20）`--test` | JPX全銘柄の下落率ランキング上位N件 | yfinance + JPX Excel（全銘柄逐次取得、数十分オーダーの実行時間になりうる。screen と同様の非機能特性） | 不要 | 実装済み（`premarket-gap`/`pts-special-quote` はデータソース未確定のため未実装、koboriakira/stock-screening#5） |
