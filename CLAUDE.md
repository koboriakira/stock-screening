...
JPX（東証）上場銘柄の**株式情報取得ハブ**（2026-07-12 に位置づけを転換）。
CLI 13コマンドのうち10がデータ取得、3が当初からの小型バリュー株スクリーニング。
DDD + TDD で開発しており、Phase 3（実データ接続）まで完了。

リポジトリ名は `stock-screening` のまま維持する（2026-08-21 判断。`koboriakira/tasks#51`）。
