...
JPX（東証）上場銘柄の**株式情報取得ハブ**。現在の CLI は13コマンドで、うち10が
データ取得（`quote` / `history` / `financials` / `signals` / `universe` / `trading-day` /
`large-holdings` / `earnings-date` / `us-index` / `decliners`）、3が当初からの
小型バリュー株スクリーニング（`screen` / `evaluate` / `diff`）です。

2026-07-12 に「小型バリュー株スクリーニングシステム」から取得ハブへ位置づけを転換しました。
リポジトリ名は `stock-screening` のままですが、これは意図した判断です
（2026-08-21。改名の利益より、Vault の32ファイルに残る旧名参照が宙に浮くコストが上回るため）。
